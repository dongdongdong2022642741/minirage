...
- **多模态文档检索（独立任务，先做最小闭环）：**
  - [x] OCR 最小闭环（2026-08-23，提前落地其中"解析入库"的文本侧）：docparser/ocr.py（RapidOCR ONNX 模型内置离线 + pypdfium2 栅格化 200dpi）；_read_pdf 对无文本层页面自动 OCR（KB_OCR=0 可关闭；单页失败降级不阻断整份文档）；真实冒烟通过（图片型 PDF 三行中文全识别）；单测 3 条覆盖启用/禁用/单页故障降级。VLM 图片描述、图文向量融合与冻结题集仍按本节原计划待做
